...
class APIProvider {
  static String base = "http://3.94.100.97:8000/";
  ...
  Future baseMultipartPersonalInfo(
    ...
    try {
      var formData;
      if(licenseFrontPhoto != null && licenseBackPhoto == null){
        String storeName = licenseFrontPhoto!.path.split("/").last;
         formData = FormData.fromMap({
                        ...body,
                        "LicenseFrontPhoto": await MultipartFile.fromFile(licenseFrontPhoto.path,
                        filename: storeName,
                        contentType:
                        m.MediaType('image', storeName.split(".").last.toLowerCase())),
                      });
      }else if(licenseFrontPhoto == null && licenseBackPhoto != null){
        String storeName = licenseBackPhoto!.path.split("/").last;
        formData = FormData.fromMap({
                        ...body,
                        "LicenseBackPhoto": await MultipartFile.fromFile(licenseBackPhoto!.path,
                        filename: storeName,
                        contentType:
                        m.MediaType('image', storeName.split(".").last.toLowerCase())),
                      });
      }else if(licenseFrontPhoto != null && licenseBackPhoto != null){
        String storeNamefront = licenseBackPhoto!.path.split("/").last;
        String storeNameback = licenseBackPhoto!.path.split("/").last;
        formData = FormData.fromMap({
                        ...body,
                        "LicenseFrontPhoto": await MultipartFile.fromFile(licenseFrontPhoto.path,
                        filename: storeNamefront,
                        contentType:
                        m.MediaType('image', storeNamefront.split(".").last.toLowerCase())),
                        "LicenseBackPhoto": await MultipartFile.fromFile(licenseBackPhoto.path,
                        filename: storeNameback,
                        contentType:
                        m.MediaType('image', storeNameback.split(".").last.toLowerCase())),
                      });
      }else{
    ...
    switch (response?.statusCode) {
      ...
      case 400:
        if (response?.data['error'] != null) {
          ...
          Utils.showToast(message: response?.data['data']['message']);
        }
  ...
  Future ordersBasePostAPI(
    ...
    try {
      Response response;
      final endpoint = fullUrl ?? (ApiEndPoints.baseUrl + url);
      final bool isMultipart = files != null && files.isNotEmpty;
      var formData;

      if (isMultipart) {
        formData = FormData.fromMap({...body,...files});
      }else{
        formData = FormData.fromMap(body);
      }
